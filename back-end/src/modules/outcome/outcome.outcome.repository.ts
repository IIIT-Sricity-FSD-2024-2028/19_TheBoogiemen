...
export class OutcomeRepository {
  ...
  async findOutcomeById(id: string): Promise<LEARNING_OUTCOME | undefined> {
    return this.outcomes.find((o) => o.outcome_id === id);
  }
  ...
  async update(
    id: string,
    data: Partial<LEARNING_OUTCOME>,
  ): Promise<LEARNING_OUTCOME | null> {
    const idx = this.outcomes.findIndex((o) => o.outcome_id === id);
    if (idx > -1) {
      Object.assign(this.outcomes[idx], data);
  ...
  async createAssessmentOutcome(
    data: ASSESSMENT_OUTCOME,
  ): Promise<ASSESSMENT_OUTCOME> {
    this.assessmentOutcomes.push(data);
    return data;
  }

  async upsertStudentOutcome(data: STUDENT_OUTCOME): Promise<STUDENT_OUTCOME> {
    const idx = this.studentOutcomes.findIndex(
      (s) =>
        s.student_id === data.student_id && s.outcome_id === data.outcome_id,
    );
    if (idx > -1) {
  ...
  async findAssessmentOutcomes(
    assessmentId: string,
  ): Promise<ASSESSMENT_OUTCOME[]> {
    return this.assessmentOutcomes.filter(
