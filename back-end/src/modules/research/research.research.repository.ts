...
export class ResearchRepository {
  ...
  async updateProject(
    id: string,
    data: Partial<RESEARCH_PROJECT>,
  ): Promise<RESEARCH_PROJECT | null> {
  ...
  async updateMilestone(
    id: string,
    data: Partial<RESEARCH_MILESTONE>,
  ): Promise<RESEARCH_MILESTONE | null> {
    const idx = this.milestones.findIndex((m) => m.milestone_id === id);
    if (idx > -1) {
      Object.assign(this.milestones[idx], data);
