...
export class CommonController {
  ...
  @Post('marks')
  @Roles('faculty', 'admin', 'head', 'superadmin')
  @ApiOperation({ summary: 'Record or update marks for a student' })
  @ApiBody({ schema: { type: 'object', additionalProperties: true } })
  async recordMarks(
    @Body() body: any,
    @CurrentUserCollegeId() actorCollegeId: string | null,
    ...
    if (existing) {
      existing.marks_obtained = Number(body.marks_obtained);
      if (body.max_marks) existing.max_marks = Number(body.max_marks);
      this.db.persist();
      return { success: true, data: existing, updated: true };
    }
    const id = `m${Date.now()}_${Math.random().toString(36).substr(2, 5)}`;
    const entry = {
      entry_id: id,
      ...body,
      marks_obtained: Number(body.marks_obtained),
      max_marks: Number(body.max_marks || 100),
      college_id: collegeId,
    };
